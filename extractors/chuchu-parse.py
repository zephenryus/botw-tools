...
def parseAAMP(data, offset=0x34, datatype='node'):
    ...
    if datatype == 'node':
        ...
        print('Reading 0x' + format(offset, '04x') + ': ' + ''.join(format(x, '02x') for x in data[offset + 4: offset + 12]))
        print(struct.unpack('<HBBHBB', data[offset + 4:offset + 12]))
        ...
        print('Reading 0x' + format(offset, '04x') + ': ' + ''.join(
            format(x, '02x') for x in data[offset + 4: offset + 8]))
        print(struct.unpack('<HBB', data[offset + 4:offset + 8]))
    ...
    # Datatypes are tentative guesses for now
        ...
        print("value:")
        print(struct.unpack('<f', data[offset:offset + 4])[0])
        ...
        try:
            print("value:")
            print(data[offset:].split(b'\0', 1)[0].decode('ascii'))
...
if __name__ == '__main__':
    f = open('../docs/Chuchu_Normal.bdrop', 'rb')
    data = f.read()
    f.close()
